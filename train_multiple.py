...
if __name__ == "__main__":
  ...
  if args.model == 'gat':
    ...
    (dataset, vocab, vocab_size,
     MULTI_BUFFER_SIZE, steps_per_epoch, MaxSeqSize) = ProcessMultilingualDataset(args)
    ...
    if os.path.isfile('{}/{}_{}_params'.format(log_dir, args.lang, args.model)):
      ...
      PARAMS = {
        ...
        "dataset_size": MULTI_BUFFER_SIZE,
        "max_tgt_length": MaxSeqSize,
        "step": 0
      }
    ...
    def eval_step(steps=None):
      ...
      for (batch, (inp, tar)) in tqdm(enumerate(
          dataset['eval_set'].take(steps))):
    ...
    def test_step():
      ...
      ref_target = []
      eval_results = open(TestResults, 'w+')
      for (batch, (inp)) in tqdm(enumerate(dataset['test_set'])):
        predictions = model(inp, targets=None, training=model.trainable)
        pred = [(predictions['outputs'].numpy().tolist())]
    ...
    if args.mode == 'train':
      ...
      for (batch, (inp, tgt)) in tqdm(enumerate(dataset['train_set'].repeat(-1))):
        if PARAMS['step'] < steps:
          start = time.time()
          PARAMS['step'] += 1
